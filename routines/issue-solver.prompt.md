...
- PRs open review-ready so the `ai-workflows` review workflows (`claude-review`, `final-pr-review`, `ai-merge-gate`) pick them up. Never auto-merge from this routine. The review workflows are the human-in-the-loop gate; AI-generated code-change PRs need that review surface active.

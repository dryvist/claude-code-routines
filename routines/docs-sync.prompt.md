...
Fetch the `docs-sync-state` gist (tracks documented concepts + open branches, so
each run is incremental, not repetitive):

```bash
gh gist list --limit 50 | grep 'docs-sync-state'
```

If none exists, create it with initial content

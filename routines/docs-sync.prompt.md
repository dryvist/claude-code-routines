...
across the estate in the last 8 days, decide where each concept belongs, and
open **up to one PR in each docs site — only where concepts actually routed**.
A quiet window opens zero PRs. Be terse. Document forward: record completed
work AND high-confidence work that was requested but not finished, as future
work.
...
- **Up to two PRs per run** — at most one in `docs` (public) and one in
  `docs-starlight` (private). Open a PR in a site ONLY if at least one concept
  routed to it this run; a quiet window opens ZERO PRs (Slack Path B reports
  counts, no PR URLs). Never manufacture a diff to justify a PR. Reuse today's
  branch if it already exists (update, don't duplicate). Both doc repos belong
  to `$DOCS_OWNER`.
...
Public docs:   [PR URL — [n] page(s) | no concepts routed, no PR]
Private docs:  [PR URL — [m] page(s) | no concepts routed, no PR]
Routed to private (sensitive/private-origin): [k] concept(s)
Future work captured: [f] item(s)
```

### Path B: quiet window (zero PRs)

```text
🟦 docs-sync — [date]
No substantive changes in the last 8 days. No PRs opened.
Repos changed: [R] · Concepts: 0 · Stale drafts closed: [s]
```

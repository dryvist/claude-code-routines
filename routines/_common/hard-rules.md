...
- Run the connectivity preflight (its own partial, included at Phase 0) immediately after the paused check and before any GitHub enumeration or state I/O. A failed preflight is a `🔴 FATAL` exit, never a "no findings" success.
...
- Respect every per-routine mutation cap stated below (max PRs / issues / labels / merges per run). Estate-wide soft cap: 2 PRs per repo per UTC day across all routines, tracked in the shared `pr-budget.json` file at the root of `$STATE_REPO` (see the state-file partial for the read/increment mechanics) — PR-opening routines whose rules below say so consult it before opening, skip the repo if at cap, otherwise increment and proceed. If that file is missing or corrupt: fail open (proceed with this routine's own per-run cap) AND emit a Slack warning.

...
## State file — `state/daily-polish.json`
...
Read the rotation state from `state/daily-polish.json` in `$STATE_REPO` using the
read + `sha`-capture pattern in the state-file partial above. Create-if-missing
initial content is `{"last_polished":"","last_date":""}` (legacy pre-v2 schema — these fields are authoritative for this routine).

If the state read fails transiently (`state_fallback=true`): per the fail-open rule, fall back to alphabetical repo order and continue.
...
Pick the first repo NOT matching the state file's `last_polished` value.
...
- **Why this PR:** <repo> was selected from the rotation (state file `state/daily-polish.json`); [N]/5 checks failed.
- **State:** `state/daily-polish.json` in `$STATE_REPO`
- **Label:** `cloud-routine`
```
...
Write the updated rotation record to `state/daily-polish.json` via the
optimistic-lock `put_state` PUT from the state-file partial above (nested
`committer`, retry once on 409). New content:

```json
{"last_polished": "<repo>", "last_date": "<today>"}
```
...
### Path C: No-op (no eligible repo, or state fallback engaged)

```text
🟦 Daily Polish — [date]

Status: no eligible repo today
Reason: [rotation cycle complete | only candidate is last_polished | state read failed → fallback engaged | all repos inactive >90 days]

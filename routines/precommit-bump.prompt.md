...
Renovate's own `pre-commit` manager covers some repos that opt in via the central preset. This routine covers the gap (repos not in the preset). The Renovate-overlap guard below ensures we never duplicate Renovate's work.
...
- Max 3 PRs per run (title suffix `[routine:precommit-bump]`).
...
- Resolve default branch SHA, create branch `chore/precommit-bump/<hook-slug>-<YYYY-MM-DD>`.
...
- **Generated by:** [precommit-bump](<PROMPT_SOURCE_URL>) — cloud routine, daily at 08:00 UTC.
...
- **State:** `state/precommit-bump.json` in `$STATE_REPO` — 14-day per-`(repo, hook)` cooldown to avoid churn.
- **Label:** `cloud-routine`
```

## Commit shape

Use the nested-committer `jq` recipe from the Hard Rules against `repos/$GH_OWNER/$REPO/contents/.pre-commit-config.yaml` (always include `sha:$sha` — the config file already exists). Commit message: `chore(deps): bump pre-commit $HOOK to $NEW_REV [routine:precommit-bump]`.

...
2. **If the frontmatter has no `trigger_id` field**, the routine has
   not yet been registered with the cloud. Auto-register it (see the
   "Auto-create" section below). On success, continue to step 3 with
   the freshly-issued `trigger_id`. On failure (HTTP error from
...
## Auto-create (for routines without a `trigger_id`)

A) Call `RemoteTrigger` with `action: create` and the body shape
   below. The `cron` value comes from the file's frontmatter; on the
   first try, use the same nested `job_config` shape used for
   `update`. Extract the new `trigger_id` from the response.

   ```json
   {
     "name": "<name from frontmatter>",
     "cron": "<cron from frontmatter>",
     "job_config": {
       "ccr": {
         "events": [{
           "data": {
   ...
   If the API rejects the shape (HTTP 4xx with a hint about required
   fields), read the error and retry with the corrected shape — do
   not retry blindly. Surface the full error in any `FAIL` line so a
   human can adjust this prompt.
